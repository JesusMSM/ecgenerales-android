...
public class BarChartRendererEC extends DataRenderer {
    ...
    public BarChartRendererEC(BarDataProvider chart, ChartAnimator animator,
            ...
            for (int i = 0; i < mChart.getBarData().getDataSetCount(); i++) {
                ...
                if (!dataSet.isStacked()) {
                    ...
                    for (int j = 0; j < valuePoints.length * mAnimator.getPhaseX(); j += 2) {
                        ...
                        drawValue(c, formattedValue, valuePoints[j], valuePoints[j + 1] + (val >= 0 ? posOffset : negOffset));
                    }
                    ...
                    for (int j = 0; j < (valuePoints.length - 1) * mAnimator.getPhaseX(); j += 2) {
                        ...
                        if (vals == null) {
                            ...
                            String formattedValue = formatter.getFormattedValue(entry.getVal(), entry, i, mViewPortHandler);
                            drawValue(c, formattedValue, valuePoints[j], valuePoints[j + 1] + (entry.getVal() >= 0 ? posOffset : negOffset));
                            ...
                            for (int k = 0; k < transformed.length; k += 2) {
                                ...
                                if (!mViewPortHandler.isInBoundsY(y)
                                    ...
                                    continue;
                                String formattedValue = formatter.getFormattedValue(vals[k / 2], entry, i, mViewPortHandler);
                                drawValue(c, formattedValue, x, y);
                            }
                        }
                    }
                }
